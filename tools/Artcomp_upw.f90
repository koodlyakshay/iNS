...
real*8              :: Re, U_inf,rho,mu,Re_l,P_inf,GradPi,GradPav,mass_l2
...
do ExtIter = 1,nExtIter
   ...
   do i=2,Nx-1
    ...
    !--- Convective terms (central) ---!    
       ...
       F_e(2) = max(rho*U_e*dy,0.d0)*U_old(2,iPoint) + min(rho*U_e*dy,0.d0)*U_old(2,jPoint) + P_e*dy ! (rho*U)*U|_e + P
       F_e(3) = max(rho*U_e*dy,0.d0)*U_old(3,iPoint) + min(rho*U_e*dy,0.d0)*U_old(3,jPoint)          ! (rho*V)*U|_e
       ...
       F_w(2) = -max(rho*U_w*dy,0.d0)*U_old(2,jPoint) - min(rho*U_w*dy,0.d0)*U_old(2,iPoint) - P_w*dy ! (rho*U)*U|_w + P
       F_w(3) = -max(rho*U_w*dy,0.d0)*U_old(3,jPoint) - min(rho*U_w*dy,0.d0)*U_old(3,iPoint)         ! (rho*V)*U|_w
       ...
       F_n(2) = max(rho*V_n*dx,0.d0)*U_old(2,iPoint) + min(rho*V_n*dx,0.d0)*U_old(2,jPoint)          ! (rho*U)*V|_n
       F_n(3) = max(rho*V_n*dx,0.d0)*U_old(3,iPoint) + min(rho*V_n*dx,0.d0)*U_old(3,jPoint) + P_n*dx ! (rho*V)*V|_n + P
       ...
       F_s(2) = -max(rho*V_s*dx,0.d0)*U_old(2,jPoint) - min(rho*V_s*dx,0.d0)*U_old(2,iPoint)         ! (rho*U)*V|_s
       F_s(3) = -max(rho*V_s*dx,0.d0)*U_old(3,jPoint) - min(rho*V_s*dx,0.d0)*U_old(3,iPoint) - P_s*dx ! (rho*V)*V|_s + P
    ...
    !--- Viscous terms ---!
       !East
       F_e(1) = 0.0
       Gr_e = 2.0*0.5*(GradU(2,1,i+1,j) + GradU(2,1,i,j))
       ...
       !West
       F_w(1) = 0.0
       Gr_w = 2.0*0.5*(GradU(2,1,i-1,j) + GradU(2,1,i,j)) 
       ...
       Gr_n = 2.0*0.5*( GradU(3,2,i,j+1) + GradU(3,2,i,j)) 
       ...
       Gr_s = 2.0*0.5*( GradU(3,2,i,j-1) + GradU(3,2,i,j)) 
   ...
   !--- Time Integration (Explicit) ---!
     ...
     enddo
     Res_l2 = sqrt(Res_l2/(Nx*Ny))
     if (modulo(ExtIter,2000) .eq. 0) print*,'Res(u): ',Res_l2,' ExtIter: ',ExtIter
   ...
   do i=2,Nx-1
    ...
    !--- Rhie-Chow interpolation ---!
       !East 
       jPoint = i+1 + (j-1)*Nx
       U_e = 0.5*( U(2,iPoint) + U(2,jPoint))
       GradPi = 0.5*(GradU(1,1,i+1,j) + GradU(1,1,i,j))
       GradPav = (U(2,jPoint) - U(2,iPoint))/dx
       F_e(1) = rho*U_e*dy + (Vol+Vol)*Area/&
      (Tot_Jac((iPoint-1)*nVar+2,(iPoint-1)*nVar+2) + Tot_Jac((jPoint-1)*nVar+2,(jPoint-1)*nVar+2))&
              *(GradPi - GradPav)
       !West
       jPoint = i-1 + (j-1)*Nx
       U_w = 0.5*( U(2,jPoint) + U(2,iPoint))
       GradPi = 0.5*(GradU(1,1,i,j) + GradU(1,1,i-1,j))
       GradPav = (U(2,jPoint) - U(2,iPoint))/dx
       F_w(1) = -rho*U_w*dy - (Vol+Vol)*Area/&
      (Tot_Jac((iPoint-1)*nVar+2,(iPoint-1)*nVar+2) + Tot_Jac((jPoint-1)*nVar+2,(jPoint-1)*nVar+2))&
              *(GradPi - GradPav)
       !North 
       jPoint = i + (j+1-1)*Nx
       U_n = 0.5*( U(3,iPoint) + U(3,jPoint))
       GradPi = 0.5*(GradU(1,2,i,j+1) + GradU(1,2,i,j))
       GradPav = (U(2,jPoint) - U(2,iPoint))/dy
       F_n(1) = rho*U_n*dx + (Vol+Vol)*Area/&
      (Tot_Jac((iPoint-1)*nVar+3,(iPoint-1)*nVar+3) + Tot_Jac((jPoint-1)*nVar+3,(jPoint-1)*nVar+3))&
              *(GradPi - GradPav)
       !South
       jPoint = i + (j-1-1)*Nx
       U_s = 0.5*( U(3,jPoint) + U(3,iPoint))
       GradPi = 0.5*(GradU(1,2,i,j) + GradU(1,2,i,j-1))
       GradPav = (U(2,jPoint) - U(2,iPoint))/dy
       F_s(1) = -rho*U_s*dx - (Vol+Vol)*Area/&
      (Tot_Jac((iPoint-1)*nVar+3,(iPoint-1)*nVar+3) + Tot_Jac((jPoint-1)*nVar+3,(jPoint-1)*nVar+3))&
              *(GradPi - GradPav)
   ...
   iPoint = i + (j-1)*Nx
       ...
       U_e = 0.5*( U(2,iPoint) + U(2,jPoint))
       GradPi = 0.5*(GradU(1,1,i+1,j) + GradU(1,1,i,j))
       GradPav = (U(1,jPoint) - U(1,iPoint))/dx
       F_e(1) = rho*U_e*dy/2.d0 + (Vol+Vol)*(Area/2.d0)/&
      (Tot_Jac((iPoint-1)*nVar+2,(iPoint-1)*nVar+2) + Tot_Jac((jPoint-1)*nVar+2,(jPoint-1)*nVar+2))&
              *(GradPi - GradPav)
       !West
       jPoint = i-1 + (j-1)*Nx
       ...
       GradPi = 0.5*(GradU(1,1,i,j) + GradU(1,1,i-1,j))
       GradPav = (U(1,jPoint) - U(1,iPoint))/dx
       F_w(1) = -rho*U_w*dy/2.d0 - (Vol+Vol)*(Area/2.d0)/&
      (Tot_Jac((iPoint-1)*nVar+2,(iPoint-1)*nVar+2) + Tot_Jac((jPoint-1)*nVar+2,(jPoint-1)*nVar+2))&
              *(GradPi - GradPav)
       !North 
       jPoint = i + (j+1-1)*Nx
       U_n = 0.5*( U(3,iPoint) + U(3,jPoint))
       GradPi = 0.5*(GradU(1,2,i,j+1) + GradU(1,2,i,j))
       GradPav = (U(1,jPoint) - U(1,iPoint))/dy
       F_n(1) = rho*U_n*dx + (Vol+Vol)*Area/&
      (Tot_Jac((iPoint-1)*nVar+3,(iPoint-1)*nVar+3) + Tot_Jac((jPoint-1)*nVar+3,(jPoint-1)*nVar+3))&
              *(GradPi - GradPav)
       !South
   ...
   iPoint = i + (j-1)*Nx
      ...
      (Tot_Jac((iPoint-1)*nVar+2,(iPoint-1)*nVar+2) + Tot_Jac((jPoint-1)*nVar+2,(jPoint-1)*nVar+2))&
       ...
       GradPav = (U(1,jPoint) - U(1,iPoint))/dy
       F_n(1) = rho*U_n*dx/2.d0 + (Vol+Vol)*(Area/2.d0)/&
      (Tot_Jac((iPoint-1)*nVar+3,(iPoint-1)*nVar+3) + Tot_Jac((jPoint-1)*nVar+3,(jPoint-1)*nVar+3))&
              *(GradPi - GradPav)
       !South
       jPoint = i + (j-1-1)*Nx
       ...
       GradPav = (U(1,jPoint) - U(1,iPoint))/dy
       F_s(1) = -rho*U_s*dx/2.d0 - (Vol+Vol)*(Area/2.d0)/&
      (Tot_Jac((iPoint-1)*nVar+3,(iPoint-1)*nVar+3) + Tot_Jac((jPoint-1)*nVar+3,(jPoint-1)*nVar+3))&
              *(GradPi - GradPav)
   ...
   Mass(iPoint) = F_w(1) + F_n(1)
   mass_l2 = 0.d0
   do iPoint = 1,nPoint
     mass_l2 = mass_l2 + Mass(iPoint)
   enddo
   !mass_l2 = sqrt(mass_l2/nPoint)
   
   if (modulo(ExtIter,2000).eq.0) print*,'Mass flux: ',mass_l2
   ...
   open(unit=14,file='Centerline_upw.txt',status='unknown')
     i = (Nx+1)/2
     do j=1,Ny
       iPoint = i + (j-1)*Nx
   ...
   if (modulo(ExtIter,2000) .eq. 0) then 
